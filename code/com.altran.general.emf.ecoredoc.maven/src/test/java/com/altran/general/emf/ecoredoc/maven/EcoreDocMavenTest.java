...
public class EcoreDocMavenTest {
	...
	public void testSpecificConfig() throws Exception {
		...
		final EPackageConfig ePackage1 = findFirst(config.getEPackages(),
				"EPackage1");
		assertNotNull(ePackage1);
		assertFalse(ePackage1.isSetRenderDefaults());
		final EClassConfig myEClass = findFirst(ePackage1.getEClasses(),
				"MyEClass");
		...
		final EAttributeConfig specialNumber = findFirst(class3.getEAttributes(),
				"specialNumber");
		...
		final EPackageConfig ePackage2 = findFirst(config.getEPackages(),
				"EPackage2");
